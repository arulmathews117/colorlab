  ...
  return (
    <div>
      ...
      <div className="back-icon">
        <FiArrowLeftCircle size={50} onClick={goBack} color="#757575" />
      </div>
      ...
      <div className="grid-parent">
        {folders?.filter((folder) => folder.userEmail == userEmail).map((filteredFolder) => (
          <>
          {filteredFolder.downloadURL !== "" ? (
          <div
          className="preview-child"
          onClick={() => openFile(filteredFolder.downloadURL)}
          key="{filteredFolder}"
          >
            {" "}
            <img
              className="image-preview"
              src={filteredFolder.downloadURL}
              alt="image"
            />
            <h5>{filteredFolder.fileName}</h5>
          </div>
        ) : (
          ""
        )}
